...
void setup_cloud_random_density_node(BaseNode *p_node)
{
...
void compute_cloud_random_density_node(BaseNode *p_node)
{
  ...
  if (p_density)
  {
    ...
    *p_out = hmap::random_cloud_density(GET("npoints", IntAttribute),
                                        density_array,
                                        GET("seed", SeedAttribute));

    if (GET_MEMBER("remap", RangeAttribute, is_active))
      p_out->remap_values(GET("remap", RangeAttribute)[0],
                          GET("remap", RangeAttribute)[1]);
  }
