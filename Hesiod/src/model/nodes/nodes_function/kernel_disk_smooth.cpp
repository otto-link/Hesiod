...
void setup_kernel_disk_smooth_node(BaseNode *p_node)
{
  ...
  ADD_ATTR(FloatAttribute, "r_cutoff", 0.9f, 0.f, 1.f);

  // attribute(s) order
  p_node->set_attr_ordered_key({"radius", "normalize", "_SEPARATOR_", "r_cutoff"});
}

void compute_kernel_disk_smooth_node(BaseNode *p_node)
{
  ...
  *p_out = hmap::disk_smooth(kernel_shape, GET("r_cutoff", FloatAttribute));
