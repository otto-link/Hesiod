...
void setup_wave_triangular_node(BaseNode *p_node)
{
  ...
  ADD_ATTR(FloatAttribute, "slant_ratio", 0.2f, 0.f, 1.f);
  ADD_ATTR(FloatAttribute, "phase_shift", 0.f, -1.f, 1.f);

  // attribute(s) order
  p_node->set_attr_ordered_key({"kw", "angle", "slant_ratio", "phase_shift"});

  setup_post_process_heightmap_attributes(p_node);
}

void compute_wave_triangular_node(BaseNode *p_node)
{
