...
void setup_path_bezier_round_node(BaseNode *p_node)
{
...
void compute_path_bezier_round_node(BaseNode *p_node)
{
  ...
  if (p_in)
  {
    ...
    *p_out = *p_in;
    ...
    if (p_in->get_npoints() > 1)
      p_out->bezier_round(GET("curvature_ratio", FloatAttribute),
