...
void setup_blend3_node(BaseNode *p_node)
{
  ...
  ADD_ATTR(EnumAttribute,
           ...
           "minimum_smooth");
  ADD_ATTR(FloatAttribute, "k1", 0.1f, 0.01f, 1.f);
  ADD_ATTR(FloatAttribute, "radius1", 0.05f, 0.f, 0.2f);

  ADD_ATTR(EnumAttribute,
           "blending_method2",
           enum_mappings.blending_method_map,
           "minimum_smooth");
  ADD_ATTR(FloatAttribute, "k2", 0.1f, 0.01f, 1.f);
  ADD_ATTR(FloatAttribute, "radius2", 0.05f, 0.f, 0.2f);

  ADD_ATTR(FloatAttribute, "input1_weight", 1.f, 0.f, 1.f);
  ADD_ATTR(FloatAttribute, "input2_weight", 1.f, 0.f, 1.f);
  ADD_ATTR(FloatAttribute, "input3_weight", 1.f, 0.f, 1.f);
  ADD_ATTR(BoolAttribute, "swap_inputs_12", false);
  ADD_ATTR(BoolAttribute, "swap_inputs_23", false);
...
void compute_blend3_node(BaseNode *p_node)
{
  ...
  {
    // adjust inputs
    if (GET("swap_inputs_12", BoolAttribute))
      std::swap(p_in1, p_in2);

    if (GET("swap_inputs_23", BoolAttribute))
    ...
    // 1 & 2
    {
      float k = GET("k1", FloatAttribute);
      int   ir = std::max(1, (int)(GET("radius1", FloatAttribute) * p_out->shape.x));
      int   method = GET("blending_method1", EnumAttribute);
    ...
    // 2 & 3
    {
      float k = GET("k2", FloatAttribute);
      int   ir = std::max(1, (int)(GET("radius2", FloatAttribute) * p_out->shape.x));
      int   method = GET("blending_method2", EnumAttribute);
      ...
      blend_heightmaps(*p_out,
                       ...
                       *p_in3,
                       ...
                       ir,
                       1.f,
                       GET("input3_weight", FloatAttribute));
    }
