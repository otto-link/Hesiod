...
NoiseSwiss::NoiseSwiss(const ModelConfig *p_config) : BaseNode(p_config)
{
  LOG_DEBUG("NoiseSwiss::NoiseSwiss");

  // model
  this->node_caption = "NoiseSwiss";
  ...
  // documentation
  this->description = "NoiseSwiss, variant of NoiseFbm.";
  ...
  this->attribute_descriptions["warp_scale"] = "Warping influence scaling.";
}

std::shared_ptr<QtNodes::NodeData> NoiseSwiss::outData(
    QtNodes::PortIndex /* port_index */)
{
  return std::static_pointer_cast<QtNodes::NodeData>(this->out);
}

void NoiseSwiss::setInData(std::shared_ptr<QtNodes::NodeData> data,
                           QtNodes::PortIndex                 port_index)
{
  if (!data)
  {
...
void NoiseSwiss::compute()
{
  ...
  hmap::fill(*p_out,
             ...
             {
               return hmap::noise_swiss((hmap::NoiseType)GET_ATTR_MAPENUM("noise_type"),
                                        shape,
                                        GET_ATTR_WAVENB("kw"),
                                        GET_ATTR_SEED("seed"),
