    ...
    : p_attr_map(p_attr_map), p_attr_ordered_key(p_attr_ordered_key)
{
  ...
  for (auto &key : attr_key_queue)
  {
    ...
    else
    {
      if (!this->p_attr_map->contains(key))
        HSDLOG->error("attribute key not found: {}", key);
      ...
      default:
        HSDLOG->error("attribute settings could not be created, for type {}",
                      (int)p_attr->get_type());
      }
      count++;
    }
  ...
  {
    HSDLOG->critical("missing attributes in AttributesWidget (check attr_ordered_key)");
