...
#define CONTROL_NODE_IMPLEMENT_FORWARD_FUNCTION_BASE(ClassName)                \
  bool ClassName ::serialize_json_v2(std::string     fieldName,                \
                                     nlohmann::json &outputData)               \
  {                                                                            \
    return ControlNode::serialize_json_v2(fieldName, outputData);              \
  }                                                                            \
  bool ClassName ::deserialize_json_v2(std::string     fieldName,              \
                                       nlohmann::json &inputData)              \
  {                                                                            \
    return ControlNode::deserialize_json_v2(fieldName, inputData);             \
  }
...
{
  for (std::map<ControlNodeType, std::string>::iterator currentIterator =
           control_node_type_to_name.begin();
       currentIterator != control_node_type_to_name.end();
       currentIterator++)
  {
...
std::unique_ptr<ControlNode> ControlNodeInstancing::create_attribute_from_type(
    ...
    void           *tree_uncast)
{
  ...
  case ControlNodeType::INVALID:
    LOG_ERROR("Encountered unknown type! (%i)", static_cast<int>(type));
    return nullptr;
  case ControlNodeType::UNARY:
    return std::unique_ptr<ControlNode>(new Unary(id));
  case ControlNodeType::BINARY:
    return std::unique_ptr<ControlNode>(new Binary(id));
  case ControlNodeType::DEBUG:
    return std::unique_ptr<ControlNode>(new Debug(id));
  case ControlNodeType::EROSION:
    return std::unique_ptr<ControlNode>(new Erosion(id));
  case ControlNodeType::FILTER:
    return std::unique_ptr<ControlNode>(new Filter(id));
  case ControlNodeType::MASK:
    return std::unique_ptr<ControlNode>(new Mask(id));
  case ControlNodeType::PRIMITIVE:
    return std::unique_ptr<ControlNode>(new Primitive(id,
                                                      tree->get_shape(),
                                                      tree->get_tiling(),
                                                      tree->get_overlap()));
  case ControlNodeType::CLONE:
    return std::unique_ptr<ControlNode>(new Clone(id));
  case ControlNodeType::ABS:
    return std::unique_ptr<ControlNode>(new Abs(id));
  case ControlNodeType::ABS_SMOOTH:
    return std::unique_ptr<ControlNode>(new AbsSmooth(id));
  case ControlNodeType::ALTER_ELEVATION:
    return std::unique_ptr<ControlNode>(new AlterElevation(id));
  case ControlNodeType::BASE_ELEVATION:
    return std::unique_ptr<ControlNode>(new BaseElevation(id,
                                                          tree->get_shape(),
                                                          tree->get_tiling(),
                                                          tree->get_overlap()));
  case ControlNodeType::BEZIER_PATH:
    return std::unique_ptr<ControlNode>(new BezierPath(id));
  case ControlNodeType::BIQUAD_PULSE:
    return std::unique_ptr<ControlNode>(new BiquadPulse(id,
                                                        tree->get_shape(),
                                                        tree->get_tiling(),
                                                        tree->get_overlap()));
  case ControlNodeType::BLEND:
    return std::unique_ptr<ControlNode>(new Blend(id));
  case ControlNodeType::BUMP:
    return std::unique_ptr<ControlNode>(new Bump(id,
                                                 tree->get_shape(),
                                                 tree->get_tiling(),
                                                 tree->get_overlap()));
  case ControlNodeType::BRUSH:
    return std::unique_ptr<ControlNode>(new Brush(id,
                                                  tree->get_shape(),
                                                  tree->get_tiling(),
                                                  tree->get_overlap()));
  case ControlNodeType::CALDERA:
    return std::unique_ptr<ControlNode>(new Caldera(id,
                                                    tree->get_shape(),
                                                    tree->get_tiling(),
                                                    tree->get_overlap()));
  case ControlNodeType::CHECKERBOARD:
    return std::unique_ptr<ControlNode>(new Checkerboard(id,
                                                         tree->get_shape(),
                                                         tree->get_tiling(),
                                                         tree->get_overlap()));
  case ControlNodeType::CLAMP:
    return std::unique_ptr<ControlNode>(new Clamp(id));
  case ControlNodeType::CLOUD:
    return std::unique_ptr<ControlNode>(new Cloud(id));
  case ControlNodeType::CLOUD_TO_ARRAY_INTERP:
    return std::unique_ptr<ControlNode>(
        new CloudToArrayInterp(id,
                               tree->get_shape(),
                               tree->get_tiling(),
                               tree->get_overlap()));
  case ControlNodeType::COLORIZE:
    return std::unique_ptr<ControlNode>(new Colorize(id));
  case ControlNodeType::COLORIZE_SOLID:
    return std::unique_ptr<ControlNode>(new ColorizeSolid(id));
  case ControlNodeType::COMBINE_MASK:
    return std::unique_ptr<ControlNode>(new CombineMask(id));
  case ControlNodeType::CONVOLVE_SVD:
    return std::unique_ptr<ControlNode>(new ConvolveSVD(id));
  case ControlNodeType::DENDRY:
    return std::unique_ptr<ControlNode>(new Dendry(id,
                                                   tree->get_shape(),
                                                   tree->get_tiling(),
                                                   tree->get_overlap()));
  case ControlNodeType::DEPRESSION_FILLING:
    return std::unique_ptr<ControlNode>(new DepressionFilling(id));
  case ControlNodeType::DIG_PATH:
    return std::unique_ptr<ControlNode>(new DigPath(id));
  case ControlNodeType::DISTANCE_TRANSFORM:
    return std::unique_ptr<ControlNode>(new DistanceTransform(id));
  case ControlNodeType::EQUALIZE:
    return std::unique_ptr<ControlNode>(new Equalize(id));
  case ControlNodeType::EROSION_MAPS:
    return std::unique_ptr<ControlNode>(new ErosionMaps(id));
  case ControlNodeType::EXPAND_SHRINK:
    return std::unique_ptr<ControlNode>(new ExpandShrink(id));
  case ControlNodeType::EXPAND_SHRINK_DIRECTIONAL:
    return std::unique_ptr<ControlNode>(new ExpandShrinkDirectional(id));
  case ControlNodeType::EXPORT:
    return std::unique_ptr<ControlNode>(new Export(id));
  case ControlNodeType::EXPORT_RGB:
    return std::unique_ptr<ControlNode>(new ExportRGB(id));
  case ControlNodeType::FACETED:
    return std::unique_ptr<ControlNode>(new Faceted(id));
  case ControlNodeType::FBM_IQ_PERLIN:
    return std::unique_ptr<ControlNode>(new FbmIqPerlin(id,
                                                        tree->get_shape(),
                                                        tree->get_tiling(),
                                                        tree->get_overlap()));
  case ControlNodeType::FBM_PERLIN:
    return std::unique_ptr<ControlNode>(new FbmPerlin(id,
                                                      tree->get_shape(),
                                                      tree->get_tiling(),
                                                      tree->get_overlap()));
  case ControlNodeType::FBM_SIMPLEX:
    return std::unique_ptr<ControlNode>(new FbmSimplex(id,
                                                       tree->get_shape(),
                                                       tree->get_tiling(),
                                                       tree->get_overlap()));
  case ControlNodeType::FBM_WORLEY:
    return std::unique_ptr<ControlNode>(new FbmWorley(id,
                                                      tree->get_shape(),
                                                      tree->get_tiling(),
                                                      tree->get_overlap()));
  case ControlNodeType::FBM_WORLEY_DOUBLE:
    return std::unique_ptr<ControlNode>(
        new FbmWorleyDouble(id,
                            tree->get_shape(),
                            tree->get_tiling(),
                            tree->get_overlap()));
  case ControlNodeType::FRACTALIZE_PATH:
    return std::unique_ptr<ControlNode>(new FractalizePath(id));
  case ControlNodeType::GABOR_NOISE:
    return std::unique_ptr<ControlNode>(new GaborNoise(id,
                                                       tree->get_shape(),
                                                       tree->get_tiling(),
                                                       tree->get_overlap()));
  case ControlNodeType::GAIN:
    return std::unique_ptr<ControlNode>(new Gain(id));
  case ControlNodeType::GAMMA_CORRECTION:
    return std::unique_ptr<ControlNode>(new GammaCorrection(id));
  case ControlNodeType::GAMMA_CORRECTION_LOCAL:
    return std::unique_ptr<ControlNode>(new GammaCorrectionLocal(id));
  case ControlNodeType::GAUSSIAN_PULSE:
    return std::unique_ptr<ControlNode>(new GaussianPulse(id,
                                                          tree->get_shape(),
                                                          tree->get_tiling(),
                                                          tree->get_overlap()));
  case ControlNodeType::GEOMORPHONS:
    return std::unique_ptr<ControlNode>(new Geomorphons(id));
  case ControlNodeType::GRADIENT:
    return std::unique_ptr<ControlNode>(new Gradient(id));
  case ControlNodeType::GRADIENT_ANGLE:
    return std::unique_ptr<ControlNode>(new GradientAngle(id));
  case ControlNodeType::GRADIENT_NORM:
    return std::unique_ptr<ControlNode>(new GradientNorm(id));
  case ControlNodeType::GRADIENT_TALUS:
    return std::unique_ptr<ControlNode>(new GradientTalus(id));
  case ControlNodeType::HYDRAULIC_ALGEBRIC:
    return std::unique_ptr<ControlNode>(new HydraulicAlgebric(id));
  case ControlNodeType::HYDRAULIC_PARTICLE:
    return std::unique_ptr<ControlNode>(new HydraulicParticle(id));
  case ControlNodeType::HYDRAULIC_RIDGE:
    return std::unique_ptr<ControlNode>(new HydraulicRidge(id));
  case ControlNodeType::HYDRAULIC_STREAM:
    return std::unique_ptr<ControlNode>(new HydraulicStream(id));
  case ControlNodeType::HYDRAULIC_STREAM_LOG:
    return std::unique_ptr<ControlNode>(new HydraulicStreamLog(id));
  case ControlNodeType::HYDRAULIC_VPIPES:
    return std::unique_ptr<ControlNode>(new HydraulicVpipes(id));
  case ControlNodeType::IMPORT:
    return std::unique_ptr<ControlNode>(new Import(id,
                                                   tree->get_shape(),
                                                   tree->get_tiling(),
                                                   tree->get_overlap()));
  case ControlNodeType::INVERSE:
    return std::unique_ptr<ControlNode>(new Inverse(id));
  case ControlNodeType::KERNEL:
    return std::unique_ptr<ControlNode>(new Kernel(id));
  case ControlNodeType::KMEANS_CLUSTERING2:
    return std::unique_ptr<ControlNode>(new KmeansClustering2(id));
  case ControlNodeType::KMEANS_CLUSTERING3:
    return std::unique_ptr<ControlNode>(new KmeansClustering3(id));
  case ControlNodeType::LAPLACE:
    return std::unique_ptr<ControlNode>(new Laplace(id));
  case ControlNodeType::LAPLACE_EDGE_PRESERVING:
    return std::unique_ptr<ControlNode>(new LaplaceEdgePreserving(id));
  case ControlNodeType::LERP:
    return std::unique_ptr<ControlNode>(new Lerp(id));
  case ControlNodeType::MAKE_BINARY:
    return std::unique_ptr<ControlNode>(new MakeBinary(id));
  case ControlNodeType::MEANDERIZE_PATH:
    return std::unique_ptr<ControlNode>(new MeanderizePath(id));
  case ControlNodeType::MEAN_LOCAL:
    return std::unique_ptr<ControlNode>(new MeanLocal(id));
  case ControlNodeType::MEDIAN3X3:
    return std::unique_ptr<ControlNode>(new Median3x3(id));
  case ControlNodeType::MINIMUM_LOCAL:
    return std::unique_ptr<ControlNode>(new MinimumLocal(id));
  case ControlNodeType::MIX_RGB:
    return std::unique_ptr<ControlNode>(new MixRGB(id));
  case ControlNodeType::NORMAL_DISPLACEMENT:
    return std::unique_ptr<ControlNode>(new NormalDisplacement(id));
  case ControlNodeType::ONE_MINUS:
    return std::unique_ptr<ControlNode>(new OneMinus(id));
  case ControlNodeType::PATH:
    return std::unique_ptr<ControlNode>(new Path(id));
  case ControlNodeType::PATH_FINDING:
    return std::unique_ptr<ControlNode>(new PathFinding(id));
  case ControlNodeType::PATH_TO_HEIGHTMAP:
    return std::unique_ptr<ControlNode>(
        new PathToHeightmap(id,
                            tree->get_shape(),
                            tree->get_tiling(),
                            tree->get_overlap()));
  case ControlNodeType::PEAK:
    return std::unique_ptr<ControlNode>(new Peak(id,
                                                 tree->get_shape(),
                                                 tree->get_tiling(),
                                                 tree->get_overlap()));
  case ControlNodeType::PERLIN:
    return std::unique_ptr<ControlNode>(new Perlin(id,
                                                   tree->get_shape(),
                                                   tree->get_tiling(),
                                                   tree->get_overlap()));
  case ControlNodeType::PERLIN_BILLOW:
    return std::unique_ptr<ControlNode>(new PerlinBillow(id,
                                                         tree->get_shape(),
                                                         tree->get_tiling(),
                                                         tree->get_overlap()));
  case ControlNodeType::PINGPONG_PERLIN:
    return std::unique_ptr<ControlNode>(
        new PingpongPerlin(id,
                           tree->get_shape(),
                           tree->get_tiling(),
                           tree->get_overlap()));
  case ControlNodeType::PLATEAU:
    return std::unique_ptr<ControlNode>(new Plateau(id));
  case ControlNodeType::PREVIEW:
    return std::unique_ptr<ControlNode>(new Preview(id));
  case ControlNodeType::PREVIEW_COLORIZE:
    return std::unique_ptr<ControlNode>(new PreviewColorize(id));
  case ControlNodeType::RECAST_CANYON:
    return std::unique_ptr<ControlNode>(new RecastCanyon(id));
  case ControlNodeType::RECAST_CLIFF:
    return std::unique_ptr<ControlNode>(new RecastCliff(id));
  case ControlNodeType::RECAST_CLIFF_DIRECTIONAL:
    return std::unique_ptr<ControlNode>(new RecastCliffDirectional(id));
  case ControlNodeType::RECAST_PEAK:
    return std::unique_ptr<ControlNode>(new RecastPeak(id));
  case ControlNodeType::RECAST_ROCKY_SLOPES:
    return std::unique_ptr<ControlNode>(new RecastRockySlopes(id));
  case ControlNodeType::RECURVE:
    return std::unique_ptr<ControlNode>(new Recurve(id));
  case ControlNodeType::RECURVE_KURA:
    return std::unique_ptr<ControlNode>(new RecurveKura(id));
  case ControlNodeType::RECURVE_S:
    return std::unique_ptr<ControlNode>(new RecurveS(id));
  case ControlNodeType::RELATIVE_ELEVATION:
    return std::unique_ptr<ControlNode>(new RelativeElevation(id));
  case ControlNodeType::REMAP:
    return std::unique_ptr<ControlNode>(new Remap(id));
  case ControlNodeType::RESCALE:
    return std::unique_ptr<ControlNode>(new Rescale(id));
  case ControlNodeType::RIDGED_PERLIN:
    return std::unique_ptr<ControlNode>(new RidgedPerlin(id,
                                                         tree->get_shape(),
                                                         tree->get_tiling(),
                                                         tree->get_overlap()));
  case ControlNodeType::RUGOSITY:
    return std::unique_ptr<ControlNode>(new Rugosity(id));
  case ControlNodeType::SEDIMENT_DEPOSITION:
    return std::unique_ptr<ControlNode>(new SedimentDeposition(id));
  case ControlNodeType::SELECT_BLOB_LOG:
    return std::unique_ptr<ControlNode>(new SelectBlobLog(id));
  case ControlNodeType::SELECT_CAVITIES:
    return std::unique_ptr<ControlNode>(new SelectCavities(id));
  case ControlNodeType::SELECT_EQ:
    return std::unique_ptr<ControlNode>(new SelectEq(id));
  case ControlNodeType::SELECT_ELEVATION_SLOPE:
    return std::unique_ptr<ControlNode>(new SelectElevationSlope(id));
  case ControlNodeType::SELECT_GRADIENT_NORM:
    return std::unique_ptr<ControlNode>(new SelectGradientNorm(id));
  case ControlNodeType::SELECT_INTERVAL:
    return std::unique_ptr<ControlNode>(new SelectInterval(id));
  case ControlNodeType::SELECT_PULSE:
    return std::unique_ptr<ControlNode>(new SelectPulse(id));
  case ControlNodeType::SELECT_RIVERS:
    return std::unique_ptr<ControlNode>(new SelectRivers(id));
  case ControlNodeType::SELECT_TRANSITIONS:
    return std::unique_ptr<ControlNode>(new SelectTransitions(id));
  case ControlNodeType::SIMPLEX:
    return std::unique_ptr<ControlNode>(new Simplex(id,
                                                    tree->get_shape(),
                                                    tree->get_tiling(),
                                                    tree->get_overlap()));
  case ControlNodeType::SLOPE:
    return std::unique_ptr<ControlNode>(new Slope(id,
                                                  tree->get_shape(),
                                                  tree->get_tiling(),
                                                  tree->get_overlap()));
  case ControlNodeType::SMOOTH_CPULSE:
    return std::unique_ptr<ControlNode>(new SmoothCpulse(id));
  case ControlNodeType::SMOOTH_FILL:
    return std::unique_ptr<ControlNode>(new SmoothFill(id));
  case ControlNodeType::SMOOTH_FILL_HOLES:
    return std::unique_ptr<ControlNode>(new SmoothFillHoles(id));
  case ControlNodeType::SMOOTH_FILL_SMEAR_PEAKS:
    return std::unique_ptr<ControlNode>(new SmoothFillSmearPeaks(id));
  case ControlNodeType::STEEPEN_CONVECTIVE:
    return std::unique_ptr<ControlNode>(new SteepenConvective(id));
  case ControlNodeType::STEP:
    return std::unique_ptr<ControlNode>(new Step(id,
                                                 tree->get_shape(),
                                                 tree->get_tiling(),
                                                 tree->get_overlap()));
  case ControlNodeType::STRATIFY_MULTISCALE:
    return std::unique_ptr<ControlNode>(new StratifyMultiscale(id));
  case ControlNodeType::STRATIFY_OBLIQUE:
    return std::unique_ptr<ControlNode>(new StratifyOblique(id));
  case ControlNodeType::THERMAL:
    return std::unique_ptr<ControlNode>(new Thermal(id));
  case ControlNodeType::THERMAL_AUTO_BEDROCK:
    return std::unique_ptr<ControlNode>(new ThermalAutoBedrock(id));
  case ControlNodeType::THERMAL_FLATTEN:
    return std::unique_ptr<ControlNode>(new ThermalFlatten(id));
  case ControlNodeType::THERMAL_SCREE:
    return std::unique_ptr<ControlNode>(new ThermalScree(id));
  case ControlNodeType::TO_KERNEL:
    return std::unique_ptr<ControlNode>(new ToKernel(id));
  case ControlNodeType::TO_MASK:
    return std::unique_ptr<ControlNode>(new ToMask(id));
  case ControlNodeType::VALLEY_WIDTH:
    return std::unique_ptr<ControlNode>(new ValleyWidth(id));
  case ControlNodeType::VALUE_NOISE_DELAUNAY:
    return std::unique_ptr<ControlNode>(
        new ValueNoiseDelaunay(id,
                               tree->get_shape(),
                               tree->get_tiling(),
                               tree->get_overlap()));
  case ControlNodeType::VALUE_NOISE_LINEAR:
    return std::unique_ptr<ControlNode>(
        new ValueNoiseLinear(id,
                             tree->get_shape(),
                             tree->get_tiling(),
                             tree->get_overlap()));
  case ControlNodeType::VALUE_NOISE_THINPLATE:
    return std::unique_ptr<ControlNode>(
        new ValueNoiseThinplate(id,
                                tree->get_shape(),
                                tree->get_tiling(),
                                tree->get_overlap()));
  case ControlNodeType::WARP:
    return std::unique_ptr<ControlNode>(new Warp(id));
  case ControlNodeType::WARP_DOWNSLOPE:
    return std::unique_ptr<ControlNode>(new WarpDownslope(id));
  case ControlNodeType::WAVE_DUNE:
    return std::unique_ptr<ControlNode>(new WaveDune(id,
                                                     tree->get_shape(),
                                                     tree->get_tiling(),
                                                     tree->get_overlap()));
  case ControlNodeType::WAVE_SINE:
    return std::unique_ptr<ControlNode>(new WaveSine(id,
                                                     tree->get_shape(),
                                                     tree->get_tiling(),
                                                     tree->get_overlap()));
  case ControlNodeType::WAVE_SQUARE:
    return std::unique_ptr<ControlNode>(new WaveSquare(id,
                                                       tree->get_shape(),
                                                       tree->get_tiling(),
                                                       tree->get_overlap()));
  case ControlNodeType::WAVE_TRIANGULAR:
    return std::unique_ptr<ControlNode>(
        new WaveTriangular(id,
                           tree->get_shape(),
                           tree->get_tiling(),
                           tree->get_overlap()));
  case ControlNodeType::WHITE:
    return std::unique_ptr<ControlNode>(new White(id,
                                                  tree->get_shape(),
                                                  tree->get_tiling(),
                                                  tree->get_overlap()));
  case ControlNodeType::WHITE_DENSITY_MAP:
    return std::unique_ptr<ControlNode>(new WhiteDensityMap(id));
  case ControlNodeType::WHITE_SPARSE:
    return std::unique_ptr<ControlNode>(new WhiteSparse(id,
                                                        tree->get_shape(),
                                                        tree->get_tiling(),
                                                        tree->get_overlap()));
  case ControlNodeType::WORLEY:
    return std::unique_ptr<ControlNode>(new Worley(id,
                                                   tree->get_shape(),
                                                   tree->get_tiling(),
                                                   tree->get_overlap()));
  case ControlNodeType::WORLEY_DOUBLE:
    return std::unique_ptr<ControlNode>(new WorleyDouble(id,
                                                         tree->get_shape(),
                                                         tree->get_tiling(),
                                                         tree->get_overlap()));
  case ControlNodeType::WORLEY_VALUE:
    return std::unique_ptr<ControlNode>(new WorleyValue(id,
                                                        tree->get_shape(),
                                                        tree->get_tiling(),
                                                        tree->get_overlap()));
  case ControlNodeType::WRINKLE:
    return std::unique_ptr<ControlNode>(new Wrinkle(id));
  case ControlNodeType::ZEROED_EDGES:
    return std::unique_ptr<ControlNode>(new ZeroedEdges(id));
  }
}
