...
NoiseSwiss::NoiseSwiss(std::string     id,
                       hmap::Vec2<int> shape,
    ...
    : ControlNode(id), Primitive(id, shape, tiling, overlap)
{
  ...
  this->attr["warp_scale"] = NEW_ATTR_FLOAT(0.2f, 0.01f, 1.f);
...
{
  LOG_DEBUG("computing NoiseSwiss node [%s]", this->id.c_str());
