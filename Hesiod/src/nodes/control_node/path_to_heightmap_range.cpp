...
PathToHeightmapRange::PathToHeightmapRange(std::string     id,
                                           hmap::Vec2<int> shape,
                                           hmap::Vec2<int> tiling,
                                           float           overlap)
    : ControlNode(id), shape(shape), tiling(tiling), overlap(overlap)
{
  LOG_DEBUG("PathToHeightmapRange::PathToHeightmapRange()");
  this->node_type = "PathToHeightmapRange";
  this->category = category_mapping.at(this->node_type);

  this->attr["aspect_ratio"] = NEW_ATTR_FLOAT(0.2f, 0.01f, 2.f);
  this->attr["remap"] = NEW_ATTR_RANGE();
  this->attr["inverse"] = NEW_ATTR_BOOL(false);

  this->attr_ordered_key = {"aspect_ratio", "inverse", "remap"};
...
void PathToHeightmapRange::compute()
{
  LOG_DEBUG("computing PathToHeightmapRange node [%s]", this->id.c_str());
  ...
  if (p_input_path->get_npoints() > 1)
  {
    hmap::fill(this->value_out,
                                    ...
                                    hmap::Array      *p_noise_y)
               {
                 ...
                 return p_input_path->to_array_range(
                     shape,
                     bbox,
...
void PathToHeightmapRange::update_inner_bindings()
{
