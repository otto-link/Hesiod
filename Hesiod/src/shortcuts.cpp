...
namespace hesiod::shortcuts
{
    ...
    ViewShortcut::ViewShortcut(std::string shortcut_label, int shortcut_key, int shortcut_modifier, Delegate shortcut_delegate, bool shortcut_enabled):
        label(shortcut_label),
        key(shortcut_key),
        modifier(shortcut_modifier),
        delegate(shortcut_delegate),
        enabled(shortcut_enabled)
    {
    }

    void ViewShortcut::PassAndCheck(int shortcut_key, int shortcut_modifier, void* pass_data)
    {
    ...
    {
        serialization::SerializationBatchHelper batch = serialization::SerializationBatchHelper();
    ...
    bool ViewShortcutsManager::RemoveAllShortcuts()
    {
        ...
        for(std::map<std::string, ViewShortcut*>::iterator currentIterator = shortcuts.begin(); currentIterator != shortcuts.end(); currentIterator++)
        {
    ...
    void ViewShortcutsManager::PassAndCheck(int shortcut_key, int shortcut_modifier, void* pass_data)
    {
        for(std::map<std::string, ViewShortcut*>::iterator currentIterator = shortcuts.begin(); currentIterator != shortcuts.end(); currentIterator++)
        {
            currentIterator->second->PassAndCheck(shortcut_key, shortcut_modifier, pass_data);
        }
    }
}