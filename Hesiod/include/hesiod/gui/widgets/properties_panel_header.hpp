...
{
  return QString(
             "QWidget#PropertiesPanelActions { background: transparent; border: none; }"
...
{
public:
  PropertiesPanelHeader(const QString         &title,
                        const meta::qt::Theme &theme,
                        QToolButton           *pin,
                        QWidget               *actions,
...
protected:
  void resizeEvent(QResizeEvent *event) override
  {
    ...
    const int title_width = std::min(
        180,
        QFontMetrics(meta::qt::ui_font(13, true)).horizontalAdvance(title_));
