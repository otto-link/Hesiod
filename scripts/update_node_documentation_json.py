...
STD_PARAM =  {
    "seed": "Random seed number. The random seed is an offset to the randomized process. A different seed will produce a new result.",
    "kw": "Base spatial frequencies in the X and Y directions. The frequencies are defined with respect to the entire domain: for example, kw = 2 produces two full oscillations across the domain width (and similarly for the Y direction).",
    "lacunarity":  "The frequency scaling factor for successive noise octaves. Higher values increase the frequency of each successive octave.",
    "octaves": "The number of octaves for fractal noise generation. More octaves add finer details to the terrain.",
    "persistence": "The amplitude scaling factor for subsequent noise octaves. Lower values reduce the contribution of higher octaves.",
    "weight": "Controls how much higher FBM octaves contribute to the noise based on local elevation. A higher weight suppresses high-frequency octaves at low elevations and increases their influence at higher elevations, producing terrain where fine details appear mainly near peaks while lower areas remain smoother.",
    #
...
STD_PORTS = {
    ...
    "dx": "Displacement with respect to the domain size (x-direction).",
    "dy": "Displacement with respect to the domain size (y-direction).",
    "envelope": "Heightmap used as a post-process amplitude multiplier for the generated noise."
}
