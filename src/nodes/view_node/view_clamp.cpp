...
bool ViewClamp::render_settings()
{
  ...
  ImGui::SliderFloat("cmin", &this->cmin, -1.f, 2.f, "%.2f");
  has_changed |= this->trigger_update_after_edit();

  ImGui::SliderFloat("cmax", &this->cmax, -1.f, 2.f, "%.2f");
  has_changed |= this->trigger_update_after_edit();
...
{
  ar(cereal::make_nvp("cmin", cmin));
  ar(cereal::make_nvp("cmax", cmax));
  ...
  ar(cereal::make_nvp("remap", remap));
